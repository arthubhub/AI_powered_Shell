...
declare -i LOGGING_STATE=1  # 1 = enabled, 0 = disabled
...
### BASH-SPECIFIC HOOK IMPLEMENTATION
# Variables to track state for preexec/precmd emulation
__ai_shell_at_prompt=0
__ai_shell_preexec_invoked=0

# Function to be called before each command (preexec equivalent)
__ai_shell_preexec_invoke() {
    # Skip if in completion
    [[ -n "$COMP_LINE" ]] && return
    
    # Skip if this is PROMPT_COMMAND itself
    [[ "$BASH_COMMAND" == "$PROMPT_COMMAND" ]] && return
    
    # Skip if we're not at a prompt
    [[ $__ai_shell_at_prompt -eq 0 ]] && return
    
    # Mark that we've invoked preexec
    __ai_shell_preexec_invoked=1
    __ai_shell_at_prompt=0
    
    # Get the actual command from history
    local this_command
    this_command="$(HISTTIMEFORMAT= history 1 | sed -e 's/^[ ]*[0-9]*[ ]*//')"
    
    # Call our preexec function
    log_command_preexec "$this_command"
}

# Function to be called before each prompt (precmd equivalent)
__ai_shell_precmd_invoke() {
    # Mark that we're at a prompt
    __ai_shell_at_prompt=1
    
    # Only call precmd if preexec was invoked (i.e., a command was actually run)
    if [[ $__ai_shell_preexec_invoked -eq 1 ]]; then
        __ai_shell_preexec_invoked=0
        log_command_precmd
    fi
}
...
if check_requirements; then
    ...
    # Set up the DEBUG trap for preexec
    trap '__ai_shell_preexec_invoke' DEBUG
    
    # Set up PROMPT_COMMAND for precmd
    # Preserve existing PROMPT_COMMAND if it exists
    if [[ -n "$PROMPT_COMMAND" ]]; then
        PROMPT_COMMAND="__ai_shell_precmd_invoke;$PROMPT_COMMAND"
    else
        PROMPT_COMMAND="__ai_shell_precmd_invoke"
    fi
